...
async function getFullVersionFromGoMod(modulePath) {
  const file = (0, import_path.join)(modulePath, "go.mod");
  let content;
  try {
    content = await (0, import_fs_extra.readFile)(file, "utf8");
  } catch (err) {
    if (err.code === "ENOENT") {
  ...
  const version2 = parseGoVersion(content);
  if (!version2) {
    console.log(`Warning: Unknown Go version in ${file}`);
    return void 0;
  }
  if (version2.major === 1 && version2.minor >= GO_MIN_VERSION) {
    return version2.patch === null ? versionMap.get(`${version2.major}.${version2.minor}`) : `${version2.major}.${version2.minor}.${version2.patch}`;
  } else {
    const versionString = version2.patch ? `${version2.major}.${version2.minor}.${version2.patch}` : `${version2.major}.${version2.minor}`;
...
function parseGoVersion(goModContent) {
